...
const PLATFORM_CONFIG = {
  ...
  redsquare_android: {
    ...
    bucket: 'apk-files',
    acceptedFiles: '.apk',
    buildInstructions: ['Build the main RedSquare mobile app for Android devices.', 'Use the automated build system or upload manually.']
  },
  redsquare_ios: {
    icon: Smartphone,
    name: 'RedSquare iOS',
    ...
    bucket: 'ios-files',
    acceptedFiles: '.ipa',
    buildInstructions: ['Build the main RedSquare mobile app for iOS devices.', 'Use the automated build system or upload manually.']
  },
  redsquare_web: {
    icon: Monitor,
    name: 'RedSquare Web',
    ...
    acceptedFiles: '.zip',
    buildInstructions: ['The web app is automatically deployed.', 'Manual builds are not typically required.']
  },
  ...
  screens_android_tv: {
    ...
    bucket: 'tv-files',
    acceptedFiles: '.apk',
    buildInstructions: ['Build the content display app for Android TV.', 'Use the automated build system for latest version.']
  },
  screens_android_mobile: {
    ...
    bucket: 'apk-files',
    acceptedFiles: '.apk',
    buildInstructions: ['Build the content display app for Android mobile devices.', 'Use the automated build system or upload manually.']
  },
  screens_ios: {
    icon: Smartphone,
    ...
    bucket: 'ios-files',
    acceptedFiles: '.ipa',
    buildInstructions: ['Build the content display app for iOS devices.', 'Use the automated build system or upload manually.']
  },
  screens_windows: {
    icon: Monitor,
    ...
    bucket: 'app_artifacts',
    acceptedFiles: '.exe',
    buildInstructions: ['Build the content display app for Windows devices.', 'Use the automated build system.']
  },
  screens_macos: {
    icon: Monitor,
    ...
    bucket: 'app_artifacts',
    acceptedFiles: '.dmg',
    buildInstructions: ['Build the content display app for macOS devices.', 'Use the automated build system.']
  },
  screens_linux: {
    icon: Monitor,
    ...
    buildInstructions: ['Build the content display app for Linux devices.', 'Use the automated build system.']
  },
  screens_amazon_fire: {
    icon: Tv,
    ...
    bucket: 'tv-files',
    acceptedFiles: '.apk',
    buildInstructions: ['Build the content display app for Amazon Fire TV.', 'Use the automated build system.']
  },
  screens_roku: {
    icon: Tv,
    ...
    bucket: 'tv-files',
    acceptedFiles: '.zip',
    buildInstructions: ['Build the content display app for Roku devices.', 'Use the automated build system.']
  },
  screens_samsung_tizen: {
    ...
    bucket: 'tv-files',
    acceptedFiles: '.tpk',
    buildInstructions: ['Build the content display app for Samsung Tizen TVs.', 'Future platform - automated build system under development.']
  },
  screens_lg_webos: {
    icon: Tv,
    ...
    bucket: 'tv-files',
    acceptedFiles: '.ipk',
    buildInstructions: ['Build the content display app for LG webOS TVs.', 'Future platform - automated build system under development.']
  }
} as const;
export const AppManager = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('app-builds-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'app_builds'
    }, payload => {
  ...
  const fetchReleases = async () => {
    try {
      ...
      const {
        data: manualReleases,
        error: manualError
      } = await supabase.from('app_releases').select('*').order('created_at', {
        ascending: false
      });
      ...
      const {
        data: automatedBuilds,
        error: automatedError
      } = await supabase.from('app_builds').select('*').in('status', ['pending', 'in_progress', 'success']).order('created_at', {
        ascending: false
      });
      ...
      const automatedReleasesFormatted = automatedBuilds.map(async build => {
        console.log('Processing automated build:', {
          id: build.id,
      ...
      const allReleases = [...manualReleasesFormatted, ...automatedReleasesFormattedResolved].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
  ...
  const fetchFileSizeWithRetry = async (url: string, maxRetries: number): Promise<number> => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('get-file-size', {
        body: {
          file_url: url
        }
      });
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const {
        data: uploadData,
        error: uploadError
      } = await supabase.storage.from(config.bucket).upload(filePath, selectedFile, {
        cacheControl: '3600',
        upsert: true
      });
    ...
    try {
      ...
      if (release.source === 'automated') {
        ...
        const {
          data,
          error
        } = await supabase.storage.from(config.bucket).createSignedUrl(release.file_path, 3600);
  ...
  const deleteRelease = async (release: AppRelease) => {
    ...
    try {
      if (isAutomated) {
        ...
        const {
          error: dbError
        } = await supabase.from('app_builds').delete().eq('id', release.build_id);
        if (dbError) throw dbError;
      } else {
        // For manual releases, delete from app_releases table
        const {
          error: storageError
        } = await supabase.storage.from(config.bucket).remove([release.file_path]);
  ...
  const handleTriggerBuild = async (app_type: 'redsquare_android' | 'redsquare_ios' | 'redsquare_web' | 'screens_android_tv' | 'screens_android_mobile' | 'screens_ios' | 'screens_windows' | 'screens_macos' | 'screens_linux' | 'screens_amazon_fire' | 'screens_roku' | 'screens_samsung_tizen' | 'screens_lg_webos') => {
    ...
    try {
      const {
        error
      } = await supabase.functions.invoke('trigger-app-build', {
        body: {
          app_type
        }
      });
    ...
    if (activePlatform === 'screens_windows' || activePlatform === 'screens_macos' || activePlatform === 'screens_linux' || activePlatform === 'redsquare_web') {
      return <>
          ...
          <Card>
            ...
            <CardContent>
              {getPlatformReleases(activePlatform).length === 0 ? <p className="text-center text-muted-foreground py-8">
                  ...
                  <TableBody>
                    {getPlatformReleases(activePlatform).map(release => <TableRow key={release.id}>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                {release.source === 'automated' && <Badge variant="secondary" className="text-xs">Auto</Badge>}
                              </div>
                              {release.source === 'automated' && release.build_id && <div className="text-xs text-muted-foreground">
                        ...
                        <TableCell>
                          {release.source === 'automated' ? <div className="flex flex-col gap-1">
                              <Badge variant={release.status === 'success' ? 'default' : release.status === 'in_progress' ? 'secondary' : release.status === 'pending' ? 'outline' : 'destructive'} className={release.status === 'success' ? 'bg-green-600 text-white' : release.status === 'in_progress' ? 'bg-blue-600 text-white animate-pulse' : release.status === 'pending' ? 'bg-yellow-600 text-white' : 'bg-red-600 text-white'}>
                                {release.status === 'in_progress' ? 'Building...' : release.status === 'pending' ? 'Queued' : release.status?.charAt(0).toUpperCase() + release.status?.slice(1)}
                              </Badge>
                              <Badge variant={release.is_active ? "default" : "secondary"} className="text-xs">
                                {release.is_active ? "Active" : "Inactive"}
                              </Badge>
                            </div> : <Badge variant={release.is_active ? "default" : "secondary"}>
                              {release.is_active ? "Active" : "Inactive"}
                            </Badge>}
                        </TableCell>
                        <TableCell>
          ...
          <Card>
            <CardHeader>
                <div className="flex justify-between items-start">
                    ...
                    <Button onClick={() => handleTriggerBuild(activePlatform)} disabled={isTriggeringBuild} variant={buildSuccessfullyStarted === activePlatform ? 'default' : 'outline'} className={buildSuccessfullyStarted === activePlatform ? 'bg-green-600 hover:bg-green-700 text-white' : ''}>
                        <Upload className="mr-2 h-4 w-4" />
                        {isTriggeringBuild ? 'Starting...' : buildSuccessfullyStarted === activePlatform ? '✓ Build Started Successfully!' : `Start Automated Build`}
                    </Button>
                </div>
            </CardHeader>
          </Card>
    ...
    return <>
                ...
                <TableBody>
                  {getPlatformReleases(activePlatform).map(release => <TableRow key={release.id}>
                       ...
                       <TableCell>
                         {release.source === 'automated' ? <div className="flex flex-col gap-1">
                             <Badge variant={release.status === 'success' ? 'default' : release.status === 'in_progress' ? 'secondary' : release.status === 'pending' ? 'outline' : 'destructive'} className={release.status === 'success' ? 'bg-green-600 text-white' : release.status === 'in_progress' ? 'bg-blue-600 text-white animate-pulse' : release.status === 'pending' ? 'bg-yellow-600 text-white' : 'bg-red-600 text-white'}>
                              ...
                              <Badge variant={release.is_active ? "default" : "secondary"} className={`text-xs ${release.is_active ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-red-600 text-white hover:bg-red-700'}`}>
                                {release.is_active ? "Active" : "Inactive"}
                              </Badge>
                           </div> : <Badge variant={release.is_active ? "default" : "secondary"}>
                             {release.is_active ? "Active" : "Inactive"}
                      ...
                      <TableCell>
                        ...
                        {release.source === 'automated' && <div className="text-xs text-muted-foreground">
                            Commit: {release.release_notes?.split('commit ')[1]?.slice(0, 7) || 'unknown'}
                          </div>}
                      </TableCell>
                      <TableCell>
        ...
        <Card>
          <CardHeader>
              <div className="flex justify-between items-start">
                  <div>
                      <CardTitle className="text-base">Automated {currentConfig.name} Build</CardTitle>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id="minimum-os-version" placeholder={activePlatform.includes('android') ? 'e.g., 7.0' : activePlatform.includes('ios') ? 'e.g., 13.0' : 'e.g., Version 10+'} value={minimumOsVersion} onChange={e => setMinimumOsVersion(e.target.value)} disabled={uploadState.isUploading} />
                </div>
                <div>
            ...
            <AlertDescription className="space-y-2">
              <p><strong>To build the {currentConfig.name} app locally:</strong></p>
                <ol className="list-decimal list-inside space-y-1 text-sm">
                  {currentConfig.buildInstructions.map((step, index) => <li key={index}>
  ...
  return <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-bold tracking-tight text-xl">App Manager</h1>
      ...
      <Tabs defaultValue="releases" className="w-full">
        ...
        <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 gap-1">
          ...
          <TabsTrigger value="redsquare_android" className="flex items-center gap-2">
            ...
            RedSquare Android
            {getPlatformReleases('redsquare_android').length > 0 && <Badge variant="secondary" className="ml-1 text-xs">
          ...
          <TabsTrigger value="redsquare_ios" className="flex items-center gap-2">
            <Smartphone className="h-4 w-4" />
            RedSquare iOS
            {getPlatformReleases('redsquare_ios').length > 0 && <Badge variant="secondary" className="ml-1 text-xs">
          ...
          <TabsTrigger value="screens_android_tv" className="flex items-center gap-2">
            ...
            {(() => {
                ...
                return screenReleases.length > 0 ? <Badge variant="secondary" className="ml-1 text-xs">
        ...
        {activePlatform.startsWith('screens_') && <div className="mt-4">
            <div className="flex flex-wrap gap-2">
              ...
              <Button variant={activePlatform === 'screens_samsung_tizen' ? 'default' : 'outline'} size="sm" onClick={() => setActivePlatform('screens_samsung_tizen')}>
                <Tv className="h-4 w-4 mr-1" />
                Samsung TV
              </Button>
              <Button variant={activePlatform === 'screens_lg_webos' ? 'default' : 'outline'} size="sm" onClick={() => setActivePlatform('screens_lg_webos')}>
                <Tv className="h-4 w-4 mr-1" />
                LG TV
              </Button>
            </div>
