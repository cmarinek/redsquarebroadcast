...
CREATE POLICY IF NOT EXISTS "Public can view active screens"
ON public.screens
FOR SELECT
USING (status = 'active');

-- Owners can view their own screens
CREATE POLICY IF NOT EXISTS "Owners can view their screens"
ON public.screens
FOR SELECT
USING (auth.uid() = owner_user_id);

-- Owners can insert their own screens
CREATE POLICY IF NOT EXISTS "Owners can insert their screens"
ON public.screens
FOR INSERT
WITH CHECK (auth.uid() = owner_user_id);

-- Owners can update their own screens
CREATE POLICY IF NOT EXISTS "Owners can update their screens"
ON public.screens
FOR UPDATE
USING (auth.uid() = owner_user_id);

-- Owners can delete their own screens
CREATE POLICY IF NOT EXISTS "Owners can delete their screens"
ON public.screens
FOR DELETE
USING (auth.uid() = owner_user_id);
