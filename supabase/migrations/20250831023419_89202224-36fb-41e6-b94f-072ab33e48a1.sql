...
DROP POLICY IF EXISTS "Allow authenticated uploads to app_artifacts" ON storage.objects;
DROP POLICY IF EXISTS "Allow public read access to app_artifacts" ON storage.objects;

-- Create RLS policy to allow authenticated users to upload artifacts
CREATE POLICY "Allow authenticated uploads to app_artifacts" 
ON storage.objects 
FOR INSERT 
WITH CHECK (bucket_id = 'app_artifacts' AND auth.role() = 'authenticated');

-- Create RLS policy to allow public read access to artifacts
CREATE POLICY "Allow public read access to app_artifacts" 
ON storage.objects 
FOR SELECT 
