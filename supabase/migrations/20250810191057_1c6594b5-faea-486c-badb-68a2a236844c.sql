-- Phase 1 core schema: bookings, payments, notifications, device_status
-- Create tables with RLS, foreign keys, indexes, and triggers
...
CREATE TABLE IF NOT EXISTS public.bookings (
  ...
  amount_cents INTEGER,                               -- optional snapshot of total price at time of booking
  currency TEXT NOT NULL DEFAULT 'USD',
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- FKs (avoid referencing auth.users directly in case of access issues)
...
CREATE POLICY IF NOT EXISTS bookings_select_for_owned_screens
ON public.bookings FOR SELECT
USING (EXISTS (
...
CREATE POLICY IF NOT EXISTS bookings_update_for_owned_screens
ON public.bookings FOR UPDATE
USING (EXISTS (
  SELECT 1 FROM public.screens s
  WHERE s.id = bookings.screen_id AND s.owner_user_id = auth.uid()
...
CREATE TABLE IF NOT EXISTS public.notifications (
  ...
  type TEXT NOT NULL DEFAULT 'general',              -- booking | payout | system | general
...
-- 4) DEVICE STATUS (used by real-time broadcasting & monitoring)
CREATE TABLE IF NOT EXISTS public.device_status (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  screen_id TEXT NOT NULL,
  device_id TEXT,
  status TEXT NOT NULL DEFAULT 'idle',              -- idle | broadcasting | offline
...
CREATE POLICY IF NOT EXISTS device_status_select_owner
...
CREATE POLICY IF NOT EXISTS device_status_insert_owner
...
CREATE POLICY IF NOT EXISTS device_status_update_owner
