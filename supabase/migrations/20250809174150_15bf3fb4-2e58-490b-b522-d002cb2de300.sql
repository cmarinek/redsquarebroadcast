-- Make migration idempotent by dropping/recreating policies when needed

-- Create screens table if missing
...
create policy screens_select_own on public.screens for select using (owner_user_id = auth.uid());

drop policy if exists screens_insert_own on public.screens;
create policy screens_insert_own on public.screens for insert with check (owner_user_id = auth.uid());

drop policy if exists screens_update_own on public.screens;
create policy screens_update_own on public.screens for update using (owner_user_id = auth.uid());

drop policy if exists screens_delete_own on public.screens;
create policy screens_delete_own on public.screens for delete using (owner_user_id = auth.uid());
...
-- Create content_schedule table and policies
...
-- Recreate content_schedule policies safely
drop policy if exists content_schedule_public_read on public.content_schedule;
create policy content_schedule_public_read on public.content_schedule for select to anon using (true);

drop policy if exists content_schedule_manage_own on public.content_schedule;
create policy content_schedule_manage_own on public.content_schedule for all to authenticated
using (exists (
  select 1 from public.screens s where s.id = content_schedule.screen_id and s.owner_user_id = auth.uid()
))
with check (exists (
  select 1 from public.screens s where s.id = content_schedule.screen_id and s.owner_user_id = auth.uid()
));
